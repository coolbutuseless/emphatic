...
To make it easier to get started with `{emphatic}` it follows the conventions 
of `{dplyr}` for selecting rows and columns,  and advanced colouring is via 
colour *scales* objects provided by `{ggplot2}` and other packages.
 
The output is focussed on the display of a data grid, highlighting user-selected
elements, but with minimal extra ["chart junk"](https://en.wikipedia.org/wiki/Chartjunk). 
This makes it suited to data exploration, but may not meet everyone's needs for
...
To add highlighing to a data.frame, specify a palette and define the rows/columns
to be highlighted:
